...
class ReportPage extends StatelessWidget {
  ...
  Future<void> generarReporteGeneral(BuildContext context) async {
    ...
    if (response.statusCode == 200) {
      ...
      var nombreArchivo = 'Reporte_General_De_Bienes_$timestam.pdf';
  ...
  Future<void> generarReporteSimple(BuildContext context) async {
    var timestam = DateTime.now();
    // Obtener la URL del servidor
    var url = Uri.parse(
        APIconstant.rutaReporteSimple); // Reemplaza con la URL del servidor
    ...
    if (response.statusCode == 200) {
      ...
      // Crear una ruta única para el archivo PDF
      var nombreArchivo = 'Reporte_por_escaneo_$timestam.pdf';
      var rutaCompleta = '${directorio!.path}/$nombreArchivo';
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 50),
            ElevatedButton(
              onPressed: () => generarReporteSimple(context),
              child: Text('Generar reporte por escaneo'),
            ),
          ],
        ),
      ),
